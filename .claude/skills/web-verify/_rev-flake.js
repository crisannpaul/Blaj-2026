...
(async () => {
  ...
  for (let i = 0; i < 8; i++) {
    ...
    // scroll the way a finger does
    await page.mouse.move(97, 300);
    await page.mouse.wheel(0, 400);
    await page.waitForTimeout(500);
    const buf = await page.screenshot();
    const s = await stats(buf, dsf);
    const bad = s.chroma > 12;
    if (bad) { fails++; await page.screenshot({ path: path.join(OUT, `c11c-flake-${i}.png`) }); }
    console.log('run', i, JSON.stringify(s), bad ? 'FROST LOST' : 'ok');
    await ctx.close();
  }
  console.log('fails', fails, '/ 8');
  await browser.close();
})();
