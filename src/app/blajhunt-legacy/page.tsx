...
/**
 * The route content lives in `src/lib/blajhunt-stops.ts` and is shared with
 * every /blajhunt-* creative variant, so that comparing two of them compares
 * their treatment and nothing else. Read the header there before changing any
 * of it — in particular what may not be lifted from the answer key.
 */
...
export default function Blajhunt() {
  return (
    ...
    >
      <main>
        {/* ── The fold ────────────────────────────────────────────────────
            The sky field is a canvas over the same gradient it draws, so a
            phone without WebGL, a failed shader link and the frame before
            first paint all show the same picture rather than a hole. */}
        <section className="relative isolate flex min-h-[68svh] flex-col overflow-hidden">
          ...
          {/* Hands the shader over to the trail's field on the SAME colour:
              this lands on --field-0 and `trail-field` starts there, so the
              two are one continuous ramp rather than two backgrounds meeting.
              Transparent at its own top edge — a gradient still opaque at the
              edge of its box draws a hard line across the page, which has
              shipped twice here. */}
          <div
            aria-hidden="true"
        ...
        {/* ── The trail ───────────────────────────────────────────────────
            Not a flat slab. `trail-field` continues the hero's own ramp down
            the page — gold where the shader hands over, through white and
            --stage, back into a soft sky — so the field stays alive for the
            4000px the reader actually scrolls. The white cards still have a
            tinted surface to lift off, which is what --stage was doing here
            before; --stage is now the middle of the ramp rather than the whole
            of it. See the utility in globals.css for the luminance bound. */}
        <section id="traseu" className="trail-field scroll-mt-4 pt-14 pb-20">
