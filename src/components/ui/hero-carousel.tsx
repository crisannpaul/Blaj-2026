...
const RATIOS = {
  narrow: {
    CARD_H: 0.37, // active card height ÷ stage height
...
export function HeroCarousel({
  ...
  // Clear air between the headline block and the top of the strip. It has to
  // outrun the headline's own descenders, which now hang TITLE_INK_BOTTOM past
  // the last line box — at a 68px title that is 8px of the gap gone before
  // anything else, and with the cards this size the two were nearly touching.
  const stripGap = Math.round(box.h * 0.045);
  ...
  const stacked = narrow
    ...
    : 0;
  const room =
    box.h * R.STRIP_TOP - topBar - stacked - stripGap - 4;
  ...
  return (
    ...
    >
      ...
      {/* ── Copy band: description and action ──
          ...
          It used to end in a position rail — "04 / 07" over a progress line.
          That was 30px plus its gap at the bottom of every stage, and at 320
          it was the single thing standing between the cards and a usable size:
          the copy floor it forced capped the card at 114px tall. Dropped at the
          user's word. What it told you is still on the page — the credit line
          under the headline names the workshop's number, and the strip shows
          its neighbours on both sides — and the picture is the subject here. */}
      <div
        className="absolute inset-x-0 bottom-0 flex flex-col justify-start"
        style={{
