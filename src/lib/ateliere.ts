 ...
 * what was delivered. The numbers are what will be printed on the day, so the
 * site shows "Atelier 02 … 09" with the gap rather than renumbering 1–7 and
 * disagreeing with every sheet of paper in Blaj.
 *
 ...
 *   - A7 has no photograph. Its card carries a stock placeholder until one
 *     arrives; `imagePlaceholder` is what lets the page say that honestly.
 */
...
export interface Atelier {
  ...
  slug: string;
  /** The organizers' number. A1 is the hunt. */
  number: number;
  ...
  tags: string[];
  /** Card image, 3:4. Under `public/ateliere/`, produced by scripts/ateliere-photos.js. */
  image: string;
...
export const ATELIERE: readonly Atelier[] = [
  ...
  },
  {
    ...
    // Stock, until the organizers send one: the Unsplash lecture-room frame
    // left over from the placeholder build, the one that at least depicts a
    // talk. The other five stock frames in public/ateliere/ are now unused
    // here; 2.jpg is still the landing page's workshops panel.
...
/** Zero-padded, the way the stage prints it: "02". */
