...
public/ateliere/              <slug>.webp — the REAL workshop cards (A3, 8 Sep), 720x960, or the
...
docs/                         the three .docx sources, and docs/ateliere/ — seven workshop
                              .docx plus poze-raw/ (what the organizers sent) and poze-org/
                              (settled artwork, already 3:4) — gitignored, see section 13
.claude/skills/               web-dev (design rubric) and web-verify (screenshot harness + reviewer)
```

| Piece | State |
|---|---|
| Landing page | one fold: marquee background — **real archive photography since 8 Sep** — title, description, and the **two branch panels** — one open, one a spine, alternating until the visitor touches anything. Copy is still placeholder; so are the two panel photographs |
| `/ateliere` | **built** — full-bleed light stage carousel, **the seven real workshops since 8 Sep**, numbered **01–07 in list order** (the organizers' own A2–A7/A9 is kept in the data as `number` but is not printed), one per card: real titles, durations, seats and hooks, and **seven settled thumbnails since 8 Sep** — generated by the user against the organizers' photographs and dropped into `poze-org/`, so no card is a stock frame any more. The CTA is *Detalii* → the workshop's own page; the focused card is mirrored into the URL hash |
...
| ~~A3~~ | ~~Workshop list: title, leader, description, location, time, capacity~~ — **delivered 8 Sep** for A2–A7 and A9 (A1 is the hunt; there is no A8): seven .docx in `docs/ateliere/` plus 18 photographs in `docs/ateliere/poze-raw/`, typed into `src/lib/ateliere.ts`. **Still owed:** coordinates — every location in the documents ends in `[GPS]` and none came, so the Maps links will be searches; the rooms for A4 and A9, which read "Sala ...."; any location at all for A7 | organizers | rooms, GPS |
...
| ~~D4~~ | ~~Do workshops need capacity limits?~~ — **answered 8 Sep by the documents**: every workshop states its seats (25–60) and four of seven split into two timed groups of 20–30. All seven run in parallel inside 13:40–15:45, so one workshop per person stands (section 5) | organizers | done |
