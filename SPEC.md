...
- `tap targets < 44px: 7` on `/blajhunt` (34 for one afternoon on 15 Sep, when each face carried a
  turn pill and the back a second title link; **7 again since that evening**,
  the pills and the back's title gone) — same class, **but it was not purely a false
...
| A6 | **Fact-check the ten stop pages.** `src/lib/blajhunt-places.ts` carries a paragraph or three of history per stop, written from general knowledge by Claude — not from a source in this repo and not by anyone who has stood in front of the buildings. Each entry is flagged `confidence: "solid"` or `"check"`, and every `"check"` entry has a `verify` line naming exactly what to confirm. **8 of 10 are `check`.** Since 15 Sep `body` is ALSO the back of the stop's card on `/blajhunt`, shown in full (a condensed `back` field lived for one afternoon and is gone), so one check covers both — and the card is read standing in the street | organizers | **before the site is public** (!) |
...
- **2026-09-15, latest (the Maps pill: grey, pin and arrow)** — the user, on
  the outlined disc: „now the pin is the blackest color from all the page,
  still looks like ass, let's use the grey for it, and can we make that
  button like a map and an arrow like -> or the pin and an arrow, so that it
  signifies the fact that it's a button and it takes you somewhere?” Both
  done. The glyphs are `--border-strong` — the ring's own grey, 3.5:1 on
  white, over the 3:1 floor for a graphic and the tone of the trailhead's
  pin — not `--muted-foreground`, which at 7.4:1 is still nearly ink; if it
  reads too faint, that is the one-class alternative. The disc is a **72 ×
  48 pill**: `MapPin` then `Arrow` (the prev/next arrows' own glyph, at
  16px), the pin for where, the arrow for going there. At 390 the pill fits
  beside the chips on all ten (278px row); at 320 it drops under them on
  four stops now (01, 02, 04, 06), not two — the pill is 24px wider than
  the disc. `audit.js` clean at 390/768/1440, same 7 title-link tap
  targets; `hittest.js` 0 unreachable.
...
- **2026-09-15, evening (the back carries everything)** — the user's polish on
  ...
  should be intuitive to click on it after you see it turn. moreover, we keep
  each stop's own page, as a safety net, but let's put all the text on the back
  of the card — we remove the title from the back so it fits — the point is to
  go into the direction that we decomission each stop's page and deliver all the
  necessary info on that card. please come back and flag if either text doesn't
  fit on the whole card, we'll find a solution for that”. Same branch, same
  port: **:3003**, with :3000 still the letter's.

  **Done.** `PEEK_DELAY` 1000 → **450** and `PEEK_MS` 1200 → **1000** (the
  module's keyframe with it). Measured after an arrow click: the card is at 1°
  at 807ms, 93° at 951, on the back from ~1160 to ~1360, 0° again by 1858 —
  the turn starts about 180ms after the 620ms travel ends and the whole
  flourish is done 1.1s later. This overrides the afternoon's BLOCKING review
  finding (the peek turning the Maps button away too soon): **the user's call,
  made having seen the 1000ms version.** Both pills are gone, `TurnMark` with
  them; the back's header keeps the number badge and the points pill and
  nothing else, and the back's title link went with the pills. `place.back` is
  **gone** from `blajhunt-places.ts`: the back renders `place.body`, the stop
  page's own paragraphs, so there is one text and one `confidence` flag (A6
  updated). Keyboard, now that there are no buttons: the `article` is
  `tabIndex 0` with a focus ring, Enter and Space turn it (or centre it, if it
  is not the centred card), Escape turns it back; it never goes inert, only
  its faces do, so the focus handoff of the afternoon is gone too. Verified at
  1440: Tab order article → title → Maps → next article; Enter → 180°, front
  inert, Tab from the turned card lands on the next card (the front's links
  unreachable); Escape → 0°; Space toggles both ways; Enter on the title link
  is the link's. Reduced motion: no peek at 1.9s, a tap swaps. **There is no
  standing sign of the back any more** — the peek, then a tap or Enter; a
  reduced-motion visitor gets neither and has the stop page behind the title.
  Accepted, at the user's direction.

  **Flagged, as asked — does the whole history fit?** `scrollHeight` against
  `clientHeight` of the back's text box (394px, sixteen 24px lines), every
  stop, four widths, in px of overflow:

  | stop | chars | 320 | 390 | 768 | 1440 |
  |---|---|---|---|---|---|
  | 01 catedrala | 629 | 230 | **110** | 0 | 0 |
  | 02 liceul | 696 | 278 | **158** | 38 | 38 |
  | 03 biserica-grecilor | 493 | 98 | 0 | 0 | 0 |
  | 04 castelul | 444 | 26 | 0 | 0 | 0 |
  | 05 casa-maniu | 415 | 2 | 0 | 0 | 0 |
  | 06 gostat | 456 | 50 | 0 | 0 | 0 |
  | 07 casa-ioan-suciu | 528 | 110 | **38** | 0 | 0 |
  | 08 colegiul | 720 | 302 | **158** | 38 | 38 |
  | 09 campia | 498 | 74 | 0 | 0 | 0 |
  | 10 protopopiatul | 405 | 0 | 0 | 0 | 0 |

  Six of ten fit at 390, the phone the site is built for; **four do not — 01,
  02, 07, 08 — by 1.5 to 6.5 lines**, and at 320 nine of ten overflow. Where
  it overflows the box scrolls behind the afternoon's edge fade, so nothing is
  hidden; but the user asked to be told, and the fix is theirs. Remedies
  tried live by injecting a style, same measurement: **(A)** the back's text
  at `--text-ui` — 13px, the ramp's own smallest, leading 1.5 — **all ten fit
  at 390 and above**, at 320 three still scroll (20/59/79px); **(B)** the card
  at 34rem: 01/02/08 still over by 46/94/94 at 390; **(C)** 36rem: 14/62/62;
  **(D)** A and B together: everything fits at 390, only 08 scrolls at 320,
  ...
  **Verified:** build and lint clean; `audit.js` 390/768/1440 — no overflow,
  ...
  title links, overlay class); `hittest.js` 0 unreachable on `/blajhunt` and
  a stop page; the front unchanged — every card 480, chips at 371, Maps at
  410 — and the mid-turn frame at 93° inside the track, nothing clipped.
  Tunables: `PEEK_DELAY` 450 and `PEEK_MS` 1000 in `trail-swipe.tsx`, the
  1000ms keyframe (40/20/40) in the module, `PEEK_BACK_WINDOW` unchanged.
...
- **2026-09-15, afternoon (the stop cards turn over)** — the user's ask, from a
  ...
  **Not done here:** :3000 still serves the main checkout (see above); Vercel
  ...
  the module. **Superseded that evening** — the entry above: both pills and
  the back's title are gone, the back carries the whole `body`, the peek is
  450/1000.
