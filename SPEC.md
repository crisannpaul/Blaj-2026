...
src/lib/                      ateliere.ts — the ten workshops, typed in from the organizers'
                                documents (A3; seven on 8 Sep, A8/A10/A11 on 14 Sep).
...
| `/ateliere` | **built** — full-bleed light stage carousel, **the ten real workshops** (seven since 8 Sep, A8/A10/A11 added 14 Sep), numbered **01–10 in list order** (the organizers' own A2–A11 is kept in the data as `number` but is not printed), one per card: real titles, durations, seats and hooks, and **ten settled thumbnails** generated by the user and dropped into `poze-org/` — no card is a stock frame. The CTA is *Detalii* → the workshop's own page; the focused card is mirrored into the URL hash |
...
| ~~A3~~ | ~~Workshop list: title, leader, description, location, time, capacity~~ — **delivered 8 Sep** for A2–A7 and A9, then **A8, A10 and A11 on 14 Sep**, closing the gap: ten .docx in `docs/ateliere/` plus 31 photographs in `docs/ateliere/poze-raw/`, typed into `src/lib/ateliere.ts`. A1 is still the hunt, so the organizers now run A2–A11 and the site 01–10 — a constant offset of one. **All ten resent 14 Sep 17:2x with a room and a `[GPS: ...]` link**, which closes everything this row had been owed since 8 Sep: every room is named (A7 and A10 had no location at all before), coordinates arrived for eight of ten, A8's seat count was corrected to 40, A11's slots cut to 50 minutes and A10 moved off 13:30. All ten thumbnails are in. **Still open, small:** A11 has a GPS link but no `Locația:` line, so its room name is read off the link (section 10.9), and A10's document still ends on "Întâlnire la ??" although its room is now named (section 10.8) | organizers | 2 minor |
