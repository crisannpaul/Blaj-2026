...
- **How the welcome letter arrives** (phase two). Phase one is the bell in the
  top-right of the fold opening the letter in a plain modal sheet
  (`src/components/ui/letter-bell.tsx`). Candidates: envelope, bell-to-sheet
  morph, iOS-style banner expanding in place, draggable bottom sheet. A banner
  variant was being built in a parallel session on 15 Sep.
