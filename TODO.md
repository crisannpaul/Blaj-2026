...
- **Fact-check the stop histories** in `src/lib/blajhunt-places.ts`. They were
  ...
  actually checked, and say who. Since 15 Sep the back of each stop's card on
  `/blajhunt` shows the same text in full, so one check covers both. Owner:
  organizers.
...
- **Three stop histories overflow the back of their card at phone width.**
  Since 15 Sep every stop card on `/blajhunt` turns over to show the whole of
  `body` from `src/lib/blajhunt-places.ts`, in a box of eighteen lines at
  390px. Measured (`scrollHeight` against `clientHeight`): catedrala runs
  66px over, liceul 114, colegiul 114; the other seven fit, and all ten fit
  from 768px up. Where it overflows the box scrolls behind an edge fade, so
  nothing is hidden. Remedies measured live, none applied: the back's text at
  `--text-ui` (13px, the ramp's smallest) fits all ten at 390 and up; that
  plus a 34rem card fits all ten at 320 as well; a taller card alone does not
  get there; or trim the three by a fifth or so (cutting is safe under the
  no-hallucination rule, adding is not). The user asked to be told and to
  pick. The stop pages `/blajhunt/[slug]` stay as the safety net meanwhile;
  the stated direction is to retire them once the card carries everything.
  Owner: user.
