 ...
 *   AL    the workshops carousel      AT-n  workshop n (site numbering 01–07)
...
add("AC-01", {
  ...
  current: "Întâlnirea Intereparhială a Tineretului",
  src: lit(F.home, "Întâlnirea Intereparhială a Tineretului"),
  deletable: true,
...
add("AC-04", {
  ...
  current:
    "La invitația Preafericitului Părinte Claudiu, tinerii din Arhieparhia de Alba Iulia și Făgăraș și din Eparhia de Cluj-Gherla petrec o zi împreună — rugăciune, voie bună, ateliere, prietenii noi și un oraș de descoperit.",
  src: lit(
    F.home,
...
add("AL-03", {
  ...
  note: "Numerotarea site-ului este 01–07, în ordinea listei. Numerele din documentele voastre (A2–A9) NU se afișează.",
});
add("AL-04", {
  ...SEC_AL,
  element:
...
ATELIERE.forEach((a, i) => {
  ...
  const SEC = {
    page: "atelier",
    section: `Atelier ${pad2(n)} — ${a.title}`,
    slug: a.slug,
    docNumber: a.number,
  };
...
for (const it of items) {
  for (const s of [].concat(it.src)) {
    if (s.kind === "literal" && !read(s.file).includes(s.text)) {
