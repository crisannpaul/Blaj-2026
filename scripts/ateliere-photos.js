/**
 * Workshop photos -> web assets.
 *
 *   cd .claude/skills/web-verify
 *   NODE_PATH=$PWD/node_modules node ../../../scripts/ateliere-photos.js
 *
 * (PowerShell: `$env:NODE_PATH="$PWD\node_modules"; node ..\..\..\scripts\ateliere-photos.js`)
 *
 ...
 *
 *   "Atelier 1 - Thumbnail.jpg"    position in the list, 1..7  -> card
 ...
 * under a 68% wash that hides any softness. The strip loads all seven up
 ...
 * ORGANIZERS' number (A1 is the hunt, there is no A8); the position in this
 * array is what "Atelier N" in a filename means.
 */
const WORKSHOPS = [
...
const RAW = [
  ...
  // says so on the card; there is deliberately no entry here for it — but see
  // STOCK below, which still gives it a backdrop.
];
 ...
 * a backdrop still is, because a stage where six slides fade to a soft field
 * and the seventh snaps into a sharp photograph reads as a bug. Delete the
 * entry when the real photograph arrives.
 */
const STOCK = [{ slug: "curajul-de-a-ti-conduce-viata", src: "5.jpg" }];
 ...
 *
 * A4 is deliberately absent: all three of its photographs are institutional
 * collages with captions baked in, and a page of them says nothing about the
 * workshop. It gets none until the organizers send a real one.
 */
const GALLERY_RAW = [
  { slug: "episcopul-tinerilor", src: ["Atelier_3_B.jpeg", "Atelier_3_C.jpeg", "Atelier_3_D.jpeg"] },
  { slug: "iconar", src: ["Atelier_5_B.jpeg", "Atelier_5_C.jpeg"] },
 ...
 * was wrong and shipped: five of the seven slugs contain dashes, so
...
function readOrg() {
  if (!fs.existsSync(SRC_ORG)) return { cards: new Map(), gallery: new Map(), skipped: [] };
  const cards = new Map();
  ...
  for (const file of fs.readdirSync(SRC_ORG).sort()) {
    ...
    if (m.role === "card") {
      if (cards.has(m.w.slug)) skipped.push(`${file} (second card for ${m.w.slug})`);
      else cards.set(m.w.slug, { file, ...m });
    } else {
      gallery.set(m.w.slug, [...(gallery.get(m.w.slug) ?? []), { file, ...m }]);
    }
  }
  return { cards, gallery, skipped };
}

/**
 * Draws one window and returns the encoded bytes plus what it did.
 *
 * `fit: "ratio"` crops to w:h — what a card needs, since the stage's 3:4 is a
 * contract. `fit: "long"` keeps the source's own ratio and only scales its
 * long edge down to `w`, which is what a gallery image needs. That second mode
 * was missing and the gallery pass called the first one with w === h, so every
 * gallery image came out square: a 1818x481 lunette was cropped to 481x481,
 * losing three quarters of its width. Never upscales in either mode.
 */
...
(async () => {
  ...
  const run = async (dir, file, out, opts) => {
    ...
    fs.writeFileSync(path.join(OUT, out), Buffer.from(res.b64, "base64"));
    return { ...res, kb: Math.round(fs.statSync(path.join(OUT, out)).size / 1024) };
  };
